...
- **PII scrubbing enforced.** `beforeSend` hooks on client, server, and edge all strip `user.email` and `user.username` before events leave the runtime. Client scrubs `ui.input` breadcrumb `message` (HTML-attr form) and `data.value` (React form). Server and edge scrub email-like substrings from breadcrumb `message` and `data` string fields across all categories. Server additionally scrubs `event.request.url`, `query_string`, `headers`, and `data` (HTTP body) the same way — Sentry's Node SDK auto-strips `Authorization`/`Cookie` but leaves custom headers and URL/body content alone, and ADR-0006 requires we close that gap at emit time.
