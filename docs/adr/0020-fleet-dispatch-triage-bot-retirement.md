...
- **Workflow updated:** `.github/workflows/claude-implementer.yml` — the `run` job (thin caller to the platform reusable workflow) adds an `if: github.event_name == 'workflow_dispatch' || github.event.sender.type != 'Bot'` guard; this blocks bot-triggered label/comment events while preserving the `workflow_dispatch` path used by the fleet promoter and human manual dispatch. The `cleanup-sweep` (Mode C) functionality is not a separate job in this file; it is driven by passing `mode: cleanup-sweep` as a `workflow_dispatch` input to the reusable workflow, which implements the Mode C logic. *(ADR originally described an `initial` job and a standalone `cleanup-sweep` job; the thin-caller architecture introduced in ADR-0034 means both are handled at the reusable-workflow layer instead.)*
