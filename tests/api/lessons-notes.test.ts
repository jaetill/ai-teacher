...
describe("POST /api/lessons/[id]/notes", () => {
  ...
  it("returns 403 when lesson belongs to a different user", async () => {
    ...
    const res = await POST(makeRequest({ notes: "Notes" }), makeParams("lesson-owned-by-other"));
  ...
  it("saves notes and returns ok when authenticated and owns the lesson", async () => {
    mockGetServerSession.mockResolvedValue({ user: { email: "teacher@example.com" } });
